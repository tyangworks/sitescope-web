...
export default function SearchVisibility({ scores }: { scores?: Scores }) {
  ...
  return <section className="my-10 border-y border-gray-800 py-8">
    ...
    <details className="mt-5 text-sm text-gray-400">
      ...
      <p className="mt-3">{scores?.scoring ? (zh ? "评分模型 v2：搜索 40、内容 30、转化信号 20、性能 10。未测量维度不扣分，也不假装通过；按已测量权重重新归一化。搜索中 SEO 占 80%，GEO 占 20%。相同观察得到相同分数，AI 不决定分数。" : "Model v2: Search 40, content 30, conversion signals 20, performance 10. Unmeasured dimensions are excluded, not failed or passed; measured weights are renormalized. Search combines 80% SEO and 20% GEO. Identical observations produce the same score, independent of AI.") : (zh ? "这份历史报告使用旧版 AI 综合评分，不能与 v2 直接比较。重新审计可获得可复算的新评分。" : "This historical report uses the legacy AI overall score and is not directly comparable with v2. Rerun the audit for a reproducible score.")}</p>
